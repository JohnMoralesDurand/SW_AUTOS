// =============================================================================
// app.routes.ts  -  MAPA DE RUTAS de la aplicacion
// -----------------------------------------------------------------------------
// Define que componente se muestra para cada URL. El router de Angular usa
// esta lista para decidir que pintar en pantalla segun la URL actual.
//
// Conceptos importantes:
//
//   LAZY LOADING (loadComponent):
//     Cada componente se descarga SOLO cuando el usuario navega a su ruta.
//     Esto reduce el tamano inicial de la aplicacion: el usuario no descarga
//     codigo de paginas que nunca va a visitar.
//
//   CHILDREN:
//     Las rutas de /app tienen un layout comun (sidebar + header) que envuelve
//     a las paginas hijas mediante el <router-outlet> del MainLayoutComponent.
//
//   GUARDS (canActivate):
//     - authGuard:  bloquea la entrada si no hay sesion iniciada.
//     - adminGuard: bloquea la entrada si el usuario no es administrador.
//
// Estructura general:
//   /                -> Landing publica
//   /login           -> Login publico
//   /register        -> Registro publico
//   /app/...         -> Todas las pantallas privadas (requieren sesion)
//   ** (cualquiera)  -> Redirige a la landing
// =============================================================================
import { Routes } from '@angular/router';
import { authGuard } from './core/guards/auth.guard';
import { adminGuard } from './core/guards/admin.guard';

export const routes: Routes = [
  // Pagina de bienvenida del taller (landing publica)
  {
    path: '',
    loadComponent: () =>
      import('./features/landing/pages/landing.component').then(
        (m) => m.LandingComponent,
      ),
  },

  // Pantallas publicas de autenticacion
  {
    path: 'login',
    loadComponent: () =>
      import('./features/auth/pages/login/login.component').then(
        (m) => m.LoginComponent,
      ),
  },
  {
    path: 'register',
    loadComponent: () =>
      import('./features/auth/pages/register/register.component').then(
        (m) => m.RegisterComponent,
      ),
  },

  // Rutas protegidas: requieren sesion iniciada
  {
    path: 'app',
    canActivate: [authGuard],
    loadComponent: () =>
      import('./shared/layout/main-layout.component').then(
        (m) => m.MainLayoutComponent,
      ),
    children: [
      {
        path: 'dashboard',
        loadComponent: () =>
          import('./features/dashboard/pages/dashboard.component').then(
            (m) => m.DashboardComponent,
          ),
      },
      {
        path: 'vehicles',
        loadComponent: () =>
          import('./features/vehicles/pages/vehicles.component').then(
            (m) => m.VehiclesComponent,
          ),
      },
      {
        // Historial de un vehículo específico (RF-35, RF-36).
        path: 'vehicles/:id/history',
        loadComponent: () =>
          import('./features/vehicles/pages/vehicle-history.component').then(
            (m) => m.VehicleHistoryComponent,
          ),
      },
      {
        path: 'services',
        loadComponent: () =>
          import('./features/services/pages/services.component').then(
            (m) => m.ServicesComponent,
          ),
      },
      {
        path: 'appointments',
        loadComponent: () =>
          import('./features/appointments/pages/appointments.component').then(
            (m) => m.AppointmentsComponent,
          ),
      },
      {
        path: 'appointments/new',
        loadComponent: () =>
          import('./features/appointments/pages/new-appointment.component').then(
            (m) => m.NewAppointmentComponent,
          ),
      },
      {
        path: 'users',
        canActivate: [adminGuard],
        loadComponent: () =>
          import('./features/users/pages/users.component').then(
            (m) => m.UsersComponent,
          ),
      },
      {
        // Pagina de reportes y estadisticas (RF-31, RF-32, RF-33).
        // Solo el administrador puede acceder.
        path: 'reports',
        canActivate: [adminGuard],
        loadComponent: () =>
          import('./features/reports/pages/reports.component').then(
            (m) => m.ReportsComponent,
          ),
      },
      {
        // Configuración de horarios del taller (admin).
        path: 'schedules',
        canActivate: [adminGuard],
        loadComponent: () =>
          import('./features/schedules/pages/schedules.component').then(
            (m) => m.SchedulesComponent,
          ),
      },
      {
        // Órdenes de trabajo (admin y mecánico).
        path: 'work-orders',
        loadComponent: () =>
          import('./features/work-orders/pages/work-orders.component').then(
            (m) => m.WorkOrdersComponent,
          ),
      },
      { path: '', pathMatch: 'full', redirectTo: 'dashboard' },
    ],
  },

  // Cualquier ruta no encontrada vuelve a la pagina de inicio
  { path: '**', redirectTo: '' },
];
